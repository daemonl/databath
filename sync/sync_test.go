...
func TestSync(t *testing.T) {
	...
	model, err := databath.ReadModelFromReader(jsonReader)

	db.Exec("DROP TABLE IF EXISTS new_table")

	db.Exec("CREATE TABLE new_table (`id` INT NOT NULL,`val` VARCHAR(45) NULL,PRIMARY KEY (`id`))")

	db.Exec("ALTER TABLE new_table ENGINE = 'MyISAM'")

	SyncDb(db, model, true)
}
